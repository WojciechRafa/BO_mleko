...
class Step:
    ...
    def detail_step(self, day: List[Tuple]) -> Tuple[bool, List]:  # funkcja zwaraca inoframcje o tym czy można wykonać funkcję oraz listę z dodatkowymi informacjami
        to_return = None
        ...
        # usuwanie
            ...
            if len(b_list) == 0:
                ...
                if car_limit > 0:
                    ...
                    max_added_milk = day[chosed_node][0].data[
...
def get_random_steps( timetable: List[List[Tuple]], n: int, max_fail_nr: int = 20) -> List:
    ...
    while len(move_list) < n:
        ...
        move_type: Step_type = Step_type(random.randrange(len(Step_type)))
        ...
        if (move_type, day_nr) in imposible_move_list:
            ...
            is_posible, data = step.detail_step(timetable[day_nr])
            if not is_posible:
                imposible_move_list.append((move_type, day_nr))
            else:
                move_list += step
